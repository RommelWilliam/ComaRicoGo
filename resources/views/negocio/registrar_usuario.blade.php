...
<body class="flex justify-end w-screen h-screen min-h-screen bg-login_img bg-cover bg-no-repeat bg-right 
    ...
    <div class="w-2/5 h-full px-10 flex flex-col justify-center items-stretch rounded-s-2xl bg-white shadow-lg 
        ...
        <h2 class="text-2xl font-bold uppercase text-center mb-8 max-md:text-xl">¡REGISTRATE CÓMO ADMINISTARDOR!</h2>
        @if(session('error'))
            <p style="color:red;">{{ session('error') }}</p>
        @endif

        <form method="POST" action="{{ route('negocio.registrarUsuario.submit') }}">
            @csrf
            ...
            <a href="{{ route('negocio.login.formulario') }}" class="text-center text-orange-500 hover:text-orange-600 hover:underline">¿Ya tienes una cuenta? Inicia sesión</a>
