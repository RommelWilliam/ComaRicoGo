...
<body class="flex justify-end w-screen h-screen min-h-screen bg-login_img bg-cover bg-no-repeat bg-right 
    ...
    <div class="w-2/5 h-full px-10 flex flex-col justify-center items-stretch rounded-s-2xl bg-white shadow-lg 
        ...
        <form method="POST" action="{{ route('negocio.login.submit') }}" id="loginForm">
            @csrf
            ...
            <input type="hidden" name="zona_horaria" id="zona_horaria">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("zona_horaria").value = Intl.DateTimeFormat().resolvedOptions().timeZone;
    });
</script>
</html>